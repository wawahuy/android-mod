adb shell am force-stop com.g4m.daitaydu

echo f | xcopy /f /Y ..\PigMod\app\build\intermediates\stripped_native_libs\release\out\lib\arm64-v8a\libpigmod.so libpigmod.so
echo f | xcopy /f /Y ..\PigMod\app\build\intermediates\dex\release\mergeDexRelease\classes.dex classes2.dex

adb push ..\PigMod\app\build\intermediates\stripped_native_libs\release\out\lib\arm64-v8a\libpigmod.so /storage/emulated/0/libpigmod.so
adb shell "su -c cp /storage/emulated/0/libpigmod.so /data/app/com.g4m.daitaydu-*/lib/arm64/libpigmod.so"
adb shell "su -c chmod 755 /data/app/com.g4m.daitaydu-*/lib/arm64/libpigmod.so"

echo f | xcopy /f /Y .\libpigmodij\.build\libpigmodij-striped.so libpigmodij.so

pscp -pw adadad libpigmodij.so yuh@10.8.0.3:/home/yuh/android-mod/PigServer/data/libso/com.g4m.daitaydu
pscp -pw adadad libpigmod.so yuh@10.8.0.3:/home/yuh/android-mod/PigServer/data/libso/libpigmod.so

@REM adb push ./libpigmodij.so /storage/emulated/0/libpigmodij.so
@REM adb shell "su -c cp /storage/emulated/0/libpigmodij.so /data/app/com.g4m.daitaydu-*/lib/arm64/"
@REM adb shell "su -c chmod 755 /data/app/com.g4m.daitaydu-*/lib/arm64/libpigmodij.so"


adb shell monkey -p com.g4m.daitaydu -c android.intent.category.LAUNCHER 1