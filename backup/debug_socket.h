...
namespace DebugSocket {
    ...
    static void (*handleMessage)(Client* client, char*, size_t);
    ...
    void *socketClientThread(void *clientVoid) {
        ...
        char buffer[DEBUG_SOCKET_BUFFER_SIZE] = { 0 };
        ssize_t valRead;

        // currently,
        // one connection
        for (auto clientPair: clients) {
            Client* clientDis = clientPair.second;
            if (clientSocket != clientDis->clientSocket) {
                close(clientDis->clientSocket);
            }
        }

        LOG_E("Start thread. %i", clientSocket);
        while (true) {
            valRead = read(clientSocket, buffer, DEBUG_SOCKET_BUFFER_SIZE - 1);
            if (valRead == 0) {
            ...
            handleMessage(client, buffer, valRead);
        }
        close(clientSocket);
        delete client;
        clients.erase(clients.find(clientSocket));
